  ...
  module Keys
    module AuthcryptKeysSelector
      def self.find_pack_sender_and_recipient_keys(frm_did_or_kid, to_did_or_kid, resolvers_config)
        frm_did, frm_kid = DID::Utils.did_or_url(frm_did_or_kid)
        to_did, to_kid = DID::Utils.did_or_url(to_did_or_kid)

        # Resolve sender DID doc
        frm_did_doc = resolvers_config.did_resolver.resolve(frm_did)
        raise DID::DocumentNotResolvedError.new(frm_did) unless frm_did_doc

        # Resolve recipient DID doc
        to_did_doc = resolvers_config.did_resolver.resolve(to_did)
        raise DID::DocumentNotResolvedError.new(to_did) unless to_did_doc
        ...
        raise DID::UrlNotFoundError, "No keyAgreement keys for sender #{frm_did}" if sender_kids.empty?
        ...
        raise DID::UrlNotFoundError, "No keyAgreement keys for recipient #{to_did}" if recipient_kids.empty?
        ...
        raise DID::SecretNotFoundError, "No sender keyAgreement secret found" unless sender_secret
        raise DID::IncompatibleCryptoError.new if compatible_recipients.empty?
      ...
      def self.find_unpack_sender_and_recipient_keys(frm_kid, to_kids, resolvers_config)
        frm_did = DID::Utils.did_from_did_url(frm_kid)
        frm_did_doc = resolvers_config.did_resolver.resolve(frm_did)
        raise DID::DocumentNotResolvedError.new(frm_did) unless frm_did_doc

        sender_vm = frm_did_doc.get_verification_method(frm_kid)
        raise DID::UrlNotFoundError, "Sender key #{frm_kid} not found" unless sender_vm

        unless frm_did_doc.key_agreement.include?(frm_kid)
          raise DID::UrlNotFoundError, "Sender key #{frm_kid} not in key_agreement"
        end

        secret_ids = resolvers_config.secrets_resolver.get_keys(to_kids)
        raise DID::SecretNotFoundError, "No recipient secrets found" if secret_ids.empty?
        ...
        raise DID::IncompatibleCryptoError.new if pairs.empty?
